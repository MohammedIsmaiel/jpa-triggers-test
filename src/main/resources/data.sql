...
CREATE TABLE
    if not exists BATCH_JOB_INSTANCE (
...
CREATE TABLE
    if not exists BATCH_JOB_EXECUTION (
...
CREATE TABLE
    if not exists BATCH_JOB_EXECUTION_PARAMS (
...
CREATE TABLE
    if not exists BATCH_STEP_EXECUTION (
...
CREATE TABLE
    if not exists BATCH_STEP_EXECUTION_CONTEXT (
        STEP_EXECUTION_ID BIGINT NOT NULL PRIMARY KEY,
...
CREATE TABLE
    if not exists BATCH_JOB_EXECUTION_CONTEXT (
        JOB_EXECUTION_ID BIGINT NOT NULL PRIMARY KEY,
        SHORT_CONTEXT VARCHAR(2500) NOT NULL,
        SERIALIZED_CONTEXT TEXT,
